...
export async function approveSwap(swapId: string, managerNote: string) {
  ...
  // Ensure the swap's shift belongs to the manager's org.
  if (!(await siteInOrg(shift.site_id, s.orgId))) return { error: "Swap request not found." };
...
export async function denySwap(swapId: string, managerNote: string) {
  ...
  const { data: shift } = await supa
    .from("shift")
    .select("site_id, date, start_time, end_time")
    .eq("id", swap.shift_id)
    .single();

  if (!shift) return { error: "Shift not found." };
  // Ensure the swap's shift belongs to the manager's org before mutating.
  if (!(await siteInOrg(shift.site_id, s.orgId))) return { error: "Swap request not found." };
